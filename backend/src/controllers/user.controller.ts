...
const userController = {
  getRecommendedUsers: asyncHandler(async (req: Request, res: Response) => {
    ...
    const recommendedUsers = await userService.getRecommendedUsers(currentUserId, friendIds);
  ...
  sendFriendRequest: asyncHandler(async (req: Request, res: Response) => {
    ...
    const existingRequest = await friendRequestService.findExistingRequest(myId, recipientId);

    if (existingRequest) {
      throw new BadRequestException('A friend request already exists between you and this user');
    }

    const friendRequest = await friendRequestService.createFriendRequest(myId, recipientId);
  ...
  acceptFriendRequest: asyncHandler(async (req: Request, res: Response) => {
    ...
    const friendRequest = await friendRequestService.findRequestById(requestId);
    ...
    if (friendRequest.recipient.toString() !== req.user.id) {
      throw new HttpException('You are not authorized to accept this request', 403);
    }
  ...
  getFriendRequests: asyncHandler(async (req: Request, res: Response) => {
    ...
    const incomingReqs = await friendRequestService.getIncomingPendingRequests(req.user.id);

    const acceptedReqs = await friendRequestService.getAcceptedOutgoingRequests(req.user.id);
  ...
  getOutgoingFriendReqs: asyncHandler(async (req: Request, res: Response) => {
    if (!req.user || !req.user._id) {
      throw new UnauthorizedException('Unauthorized: user not found in request');
    }

    const outgoingRequests = await friendRequestService.getOutgoingPendingRequests(req.user.id);
