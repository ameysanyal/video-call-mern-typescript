...
export interface IFriendRequestService {
  ...
  findExistingRequest(senderId: string | Types.ObjectId, recipientId: string | Types.ObjectId): Promise<FriendRequestDocument | null>;

  // Corresponds to: const friendRequest = await FriendRequest.create(...)
  createFriendRequest(senderId: string | Types.ObjectId, recipientId: string | Types.ObjectId): Promise<FriendRequestDocument>;
  ...
  updateRequestStatus(requestId: string | Types.ObjectId, status: 'accepted' | 'pending' | 'rejected'): Promise<FriendRequestDocument | null>;
}
...
export class FriendRequestService implements IFriendRequestService {
   ...
   */
  async findExistingRequest(senderId: string | Types.ObjectId, recipientId: string | Types.ObjectId): Promise<FriendRequestDocument | null> {
    this.logger.debug(`Checking for existing friend request between ${senderId} and ${recipientId}`);
    try {
      ...
      this.logger.error(`Error finding existing friend request between ${senderId} and ${recipientId}:`, error);
      throw error;
    }
  }
   ...
   */
  async createFriendRequest(senderId: string | Types.ObjectId, recipientId: string | Types.ObjectId): Promise<FriendRequestDocument> {
   ...
   */
  async getIncomingPendingRequests(userId: string | Types.ObjectId): Promise<FriendRequestDocument[]> {
   ...
   */
  async getAcceptedOutgoingRequests(userId: string | Types.ObjectId): Promise<FriendRequestDocument[]> {
   ...
   */
  async getOutgoingPendingRequests(userId: string | Types.ObjectId): Promise<FriendRequestDocument[]> {
   ...
   */
  async updateRequestStatus(requestId: string | Types.ObjectId, status: 'accepted' | 'pending' | 'rejected'): Promise<FriendRequestDocument | null> {
