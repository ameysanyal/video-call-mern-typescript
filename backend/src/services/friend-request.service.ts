...
} from "@/models/friend-request.model.js"; // Adjust path if necessary
...
export class FriendRequestService implements IFriendRequestService {
  ...
  async createFriendRequest(
    ...
    try {
      ...
      this.logger.error(
        `Error creating friend request from ${senderId} to ${recipientId}:`,
        error
      );
      throw error;
    }
  ...
  async findRequestById(
    ...
    try {
      ...
      this.logger.error(
        `Error finding friend request by ID ${requestId}:`,
        error
      );
      throw error;
    }
  ...
  async getIncomingPendingRequests(
    ...
    this.logger.debug(
      `Fetching incoming pending friend requests for user ${userId}`
    );
    try {
      ...
      this.logger.error(
        `Error fetching incoming pending requests for user ${userId}:`,
        error
      );
      throw error;
    }
  ...
  async getAcceptedOutgoingRequests(
    ...
    this.logger.debug(
      `Fetching accepted outgoing friend requests from user ${userId}`
    );
    try {
      return await this.friendRequestModel
        .find({
      ...
      this.logger.error(
        `Error fetching accepted outgoing requests from user ${userId}:`,
        error
      );
      throw error;
    }
  ...
  async getOutgoingPendingRequests(
    ...
    this.logger.debug(
      `Fetching outgoing pending friend requests from user ${userId}`
    );
    try {
      return await this.friendRequestModel
        .find({
      ...
      this.logger.error(
        `Error fetching outgoing pending requests from user ${userId}:`,
        error
      );
      throw error;
    }
  ...
  async updateRequestStatus(
    ...
    this.logger.info(
      `Updating status of friend request ${requestId} to ${status}`
    );
    try {
      ...
      this.logger.error(
        `Error updating status for friend request ${requestId}:`,
        error
      );
      throw error;
    }
